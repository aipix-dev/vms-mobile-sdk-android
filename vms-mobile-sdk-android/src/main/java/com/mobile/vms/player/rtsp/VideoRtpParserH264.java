...
class VideoRtpParserH264 {
    private static final String TAG = VideoRtpParserH264.class.getSimpleName();
...
//        logSdk(TAG, "processRtpPacketAndGetNalUnit(length=" + length + ")");

        int tmpLen;
        int nalType = data[0] & 0x1F;
        int packFlag = data[1] & 0xC0;

//        logSdk(TAG, "NAL type: " + nalType + ", pack flag: " + packFlag);
        switch (nalType) {
