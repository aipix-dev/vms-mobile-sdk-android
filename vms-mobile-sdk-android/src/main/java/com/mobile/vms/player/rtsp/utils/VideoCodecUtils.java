...
// Table 7-3: NAL unit type codes
    ...
    // 39: // supplemental enhancement information (SEI)
    private static final byte[] NAL_PREFIX1 = {0x00, 0x00, 0x00, 0x01};
    private static final byte[] NAL_PREFIX2 = {0x00, 0x00, 0x01};

    /**
     * Получить тип NAL единицы для H265 потока.
     *
     * @param data   Массив байтов, содержащий NAL единицу.
     * @param offset Смещение в массиве, с которого начинается NAL единица.
     * @param length Длина данных, доступных для анализа.
     * @return Тип NAL единицы в виде байта.
     */
    public static byte getNalUnitType(@Nullable byte[] data, int offset, int length) {
...
//            return (byte) (nalUnitTypeOctet & 0x1f); // todo check both 264 & 265
            return (byte) ((nalUnitTypeOctet & 0x7E) >> 1); // Используем 6 бит для определения типа NAL единицы
        } else {
            return (byte)-1;
        }
    }
...
//    }
    ...
    public static boolean isHEVCSupported() {
        // Перебор всех доступных кодеков на устройстве
        for (int i = 0; i < MediaCodecList.getCodecCount(); i++) {
            MediaCodecInfo codecInfo = MediaCodecList.getCodecInfoAt(i);
            // Проверка, является ли кодек декодером
            if (!codecInfo.isEncoder()) {
                String[] types = codecInfo.getSupportedTypes();
                for (String type : types) {
                    // Проверка поддержки MIME типа video/hevc
                    if (type.equalsIgnoreCase("video/hevc")) {
                        Log.d(TAG, "isHEVCSupported +++ has type = " + type);
                        return true; // H265 (HEVC) поддерживается
                    }
                }
            }
        }
        Log.d(TAG, "isHEVCSupported --- doesn't have video/hevc");
        return false; // H265 (HEVC) не поддерживается
    }
